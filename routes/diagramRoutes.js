...
// Ruta para eliminar un diagrama por su ID
router.delete("/eliminar/:id", (req, res) => {
  const diagramaId = req.params.id;
  
  // Verificar si el diagrama existe y eliminarlo
  Diagram.findByIdAndDelete(diagramaId)
    .then((diagrama) => {
      if (!diagrama) {
        return res.status(404).json({ message: "Diagrama no encontrado" });
      }
      res.status(200).json({ message: "Diagrama eliminado exitosamente" });
    })
    .catch((error) => {
